...
namespace sylar{
    ...
    //string->vector
    template <class T>
    class LexcicalCast<std::string, std::vector<T>> {
    public:
    ...
    template<class T
            ,class FromStr = LexcicalCast<std::string, T>
            ,class ToStr = LexcicalCast<T, std::string>>
...
//                m_val = boost::lexical_cast<T>(val);
                ...
                SYLAR_LOG_ERROR(SYLAR_LOG_ROOT()) << "ConfigVar::toString exception"
                    << e.what() << " convert: string type: " << typeid(m_val).name();
            }
            return false;
        }
